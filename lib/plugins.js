...
const recursivelyResetModuleCache = depPath => {
  let mod = require.resolve(depPath);

  if (mod && mod in require.cache) {
    mod = require.cache[mod];

    const fn = mod => {
      mod.children.forEach(child => fn(child));

      delete require.cache[mod.id];

      recursivelyClearObject(mod);
    };
...
const recursivelyClearObject = obj => {
  Object.keys(obj).forEach(key => {
    if (obj[key] instanceof Object) {
      recursivelyClearObject(obj[key]);
    } else if (key !== 'paths') {
      delete obj[key];
    }
  });
